...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('stats-legacy')
    .setDescription('Get the detailed ELO stats for a player (legacy version)')
  ...
  async autocomplete(interaction) {
    ...
    console.log(`[${timestamp}] Processing stats-legacy autocomplete:
    ...
    try {
      ...
      console.log(`[${timestamp}] Stats-legacy autocomplete returned ${results.length} results for user ${user.tag} (${user.id}) search: "${searchTerm}"`);
    } catch (error) {
  ...
  async execute(interaction) {
    ...
    console.log(`[${timestamp}] Executing stats-legacy command:
    User: ${user.tag} (${user.id})
    ...
    try {
      ...
      if (playerRows.length === 0) {
        console.log(`[${timestamp}] Player ${playerName} not found for stats-legacy requested by ${user.tag} (${user.id})`);
      ...
      const embed = {
        color: 0x0099ff,
        title: `📊 Detailed Stats for ${playerName}`,
        fields: [],
        footer: { text: 'DFC Stats Legacy' },
      };
      ...
      console.log(`[${timestamp}] Stats-legacy for player ${playerName} sent successfully to ${user.tag} (${user.id}) - found ${processedMatchTypes.size} match types`);
    } catch (error) {
      const errorMessage = `[${timestamp}] Error fetching stats-legacy for player ${playerName} requested by ${user.tag} (${user.id})`;
      console.error(errorMessage, error);
