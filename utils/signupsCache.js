...
const CACHE_KEY = 'dfc-data:recent-signups';
const CACHE_TIMESTAMP_KEY = 'dfc-data:recent-signups-timestamp';
...
class SignupsCache {
    ...
    async getCachedData() {
        try {
            ...
            if (cachedData) {
                console.log('Retrieved signup data from Redis cache');
                return JSON.parse(cachedData);
            } else {
                console.log('No cached signup data found, attempting to refresh cache');
    ...
    async refreshCache() {
        if (this.isRefreshing) {
            console.log('Signup cache refresh already in progress, waiting...');
        ...
        try {
            console.log('Refreshing Recent Signups cache...');
            ...
            try {
                ...
                if (client && redisClient.isReady()) {
                    ...
                    console.log(`Signup cache refreshed with ${data.length} rows, TTL: ${CACHE_TTL}s (3 hours)`);
                } else {
            ...
            console.error('Error refreshing signup cache:', error);
            throw error;
        } finally {
            this.isRefreshing = false;
        }
    ...
    async getCacheTimestamp() {
        try {
            ...
            console.error('Error getting signup cache timestamp:', error);
            return null;
        }
    }
...
module.exports = new SignupsCache();
